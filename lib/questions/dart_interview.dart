...
print(identical(list1, list2)); // true - экономия памяти!
  ...
  QA(
    ...
    q: 'Что такое Future и async/await в Dart?',
    a: 'Future представляет значение, которое будет получено в будущем (например, результат асинхронной операции). Ключевые слова async и await упрощают работу с Future: async обозначает функцию, возвращающую Future, а await "приостанавливает" выполнение до получения результата.',
  ),
  QA(
    tags: [Tag.dart],
...
`??` возвращает правый операнд, если левый равен null. Пример: `a ?? b`.

Оператор `??=` присваивает значение переменной только если она равна null, иначе оставляет существующее значение''',
  ),
  QA(
    tags: [Tag.dart],
...
var → тип определяется компилятором один раз и становится фиксированным, дальше изменить его нельзя. Ошибки будут на этапе компиляции.
dynamic → переменная может хранить значения любых типов в любой момент, проверки типов откладываются на рантайм. Ошибки ловятся только при выполнении.
...
dynamic - Отсутствие проверок времени компиляции:
...
1. for - когда нужен индекс или доступ по нему.
...
''',
  ...
  QA(
    ...
    a: '`const`-конструкторы позволяют создавать compile-time константные объекты, которые повторно используются. Это снижает нагрузку на память и повышает производительность. Const-конструкторы позволяют Dart переиспользовать один и тот же экземпляр объекта при одинаковых параметрах, что снижает расход памяти.',
  ),
  QA(
    tags: [Tag.dart],
  ...
  QA(
    tags: [Tag.dart],
    q: 'Как работает cascade operator (..)?',
    a: 'Оператор `..` позволяет вызывать несколько методов на одном объекте без промежуточных переменных. Пример: `myObject..method1()..method2();`.',
  ),
  QA(
    tags: [Tag.dart],
...
''',
  ...
  QA(
    ...
    a: 'Создать immutable-объект можно с помощью `final` полей и `const` конструктора. Также популярны пакеты вроде `freezed`, которые позволяют автоматически генерировать immutable классы.',
  ),
  QA(
    tags: [Tag.dart],
    q: 'Что такое typedef?',
    a: '''
`typedef` используется для создания псевдонима типа функции. Это упрощает чтение кода. Пример: `typedef IntCallback = void Function(int value);`.
...
''',
  ...
  QA(
    tags: [Tag.dart],
    q: 'Как работает factory консткрутор?',
    a: r'''
Factory-конструктор в Dart предоставляет полный контроль над процессом создания объекта и может:
...
''',
  ...
  QA(
    ...
    a: '1. Использовать `const` для литералов: `const list = [1, 2]`.\n2. Использовать Unmodifiable классы из `package:collection`, например `UnmodifiableListView`.\n3. С Dart 2.17+: `List.unmodifiable()`',
  ),
  QA(
    tags: [Tag.dart],
    q: 'Что произойдёт, если обратиться к `late` переменной до инициализации?',
    a: 'Будет выброшано `LateInitializationError`. Это предотвращает обращение к переменным до их назначения.',
  ),
  QA(
    tags: [Tag.dart],
...
''',
  ...
  QA(
    tags: [Tag.dart],
    q: 'Почему стоит избегать использования `dynamic`, когда это возможно?',
    a: 'Тип `dynamic` отключает проверку типов во время компиляции, что увеличивает риск runtime-ошибок. Лучше использовать точные или обобщённые типы для type safety и читаемости.',
  ),
  QA(
    tags: [Tag.dart],
...
2. @deprecated - IDE и анализатор предупреждают разработчиков об использовании устаревшего кода.
...
class Car extends Vehicle {
  void drive() {
    startEngine(); // ✅ Это правильно - использование в подклассе
  }
}
void main() {
  var car = Car();
  car.drive(); // ✅ Правильно: использовать через публичный интерфейс
}
...
Рефлексия позволяет исследовать структуру объектов во время выполнения (`dart:mirrors`).
...
''',
  ...
  QA(
    ...
    a: 'Можно использовать оператор spread (`...`): `[...list1, ...list2]`, метод `addAll()`, а также `List.from(list1)..addAll(list2)`. Для защиты от null — `...?list2`.',
  ),
  QA(
    tags: [Tag.dart],
...
''',
  ...
  QA(
    tags: [Tag.dart],
    q: 'Как работает оператор `!` (null assertion) в Dart?',
  ...
  QA(
    tags: [Tag.dart, Tag.future],
    q: 'Какой порядок выполнения у синхронных и асинхронных операций?',
    a: '''
Порядок выполнения async-функций основан на event loop и очереди событий, и выглядит следующим образом:

📌 Порядок выполнения:
1. Синхронный код выполняется сразу.
2. Когда выполнение доходит до await, управление возвращается в event loop: Всё, что после await, ставится в очередь микрозадач (microtask queue).
3. Microtasks исполняются до следующего события из основной event queue.
4. После выполнения всех микрозадач, event loop берёт задачу из обычной очереди событий (например, таймеры, IO и т.д.).

⚠️ Подвохи:
- scheduleMicrotask попадает в microtask queue → выполняется до обычных Future.
- Future() (без delayed) тоже попадает в microtask.
- Future.delayed(Duration.zero) ставится в обычную event queue, несмотря на "zero".
- Isolate сообщения выполняются в event queue
...
''',
  ...
  QA(
    ...
    a: '`assert()` используется во время разработки для проверки предусловий. Если условие ложно — выбрасывается исключение. В релизной сборке `assert()` не выполняется.',
  ),
  QA(
    tags: [Tag.dart],
  ...
  QA(
    ...
    a: 'Избегать долгоживущих слушателей (`StreamSubscription`, `Timer`, `controller`), вручную отписываться (`cancel()`), использовать `dispose()` и `finalize()` в соответствующих местах.',
  ),
  QA(
    tags: [Tag.dart],
...
''',
  ...
  QA(
    tags: [Tag.dart],
    q: 'Rак создать абстрактные переменные и методы в абстрактном классе?',
    a: '''
...
abstract class Vehicle {
  ...
  final String type = 'vehicle'; // Конкретная переменная (с реализацией)
}
...
''',
  ...
  QA(
    tags: [Tag.dart],
    q: 'Что такое `late final` и чем оно отличается от `final`?',
    a: '`late final` — переменная, которая может быть инициализирована позже, но только один раз. Полезна, когда значение зависит от других данных, доступных позже (например, после конструкторов).',
  ),
  QA(
    tags: [Tag.dart],
...
⏱️ readAsStringSync() - Синхронный метод
...
''',
  ...
  QA(
    tags: [Tag.dart],
    q: 'Как сериализовать/десериализовать объекты в JSON в Dart?',
...
''',
  ...
  QA(
    ...
    a: '`const []` создаёт compile-time константный список. `List.empty(growable: false)` — тоже неизменяемый, но создаётся во время runtimeType и не является const.',
  ),
  QA(
    tags: [Tag.dart],
...
''',
  ...
  QA(
    tags: [Tag.dart],
    q: 'Как избежать удержания объектов в памяти?',
    a: '''
_controller.dispose();
...
Dependency overrides (временное решение):
- dependency_overrides:
    package_name: 2.0.0

Обновить constraints всех зависимостей до совместимых версий
Форк проблемной зависимости и обновить её
Ждать обновления от мейнтейнеров
        ''',
  ),
  QA(
    tags: [Tag.dart],
...
''',
  ...
  QA(
    q: 'Что такое Zones и runZonedGuarded?',
    a: r'''

runZoned - Создает зону (изолированную область выполнения) с возможностью перехвата синхронных ошибок и управления контекстом.
Устаревший
...
runZonedGuarded - Рекомендуемый способ - перехватывает и синхронные, и асинхронные ошибки.
Асинхронные ошибки перехватывает
рекомендуется
...
''',
  ...
  QA(
    ...
    q: 'Изоляты в Dart общаются копированием или ссылками?',
    a: '''
...
Проблема без TransferableTypedData:

// Обычная передача данных между изолятами
...
void main() async {
  ...
  final Uint8List originalData = Uint8List(5);
  for (var i = 0; i < originalData.length; i++) {
    originalData[i] = i + 1;
  }

  print('Main Isolate:');
  print('Оригинал: ${originalData.toList()}');
  print('hashCode оригинала: ${originalData.hashCode}');

  await Isolate.spawn(worker, receivePort.sendPort);

  // Получаем SendPort воркера
  final sendPort = await receivePort.first as SendPort;

  // Создаем порт для ответа
  final answerPort = ReceivePort();

  // Отправляем копируемые данные
  sendPort.send([originalData, answerPort.sendPort]);

  // Получаем ответ
  final copiedData = await answerPort.first as Uint8List;

  print('\nMain Isolate после передачи:');
  print('Оригинал: ${originalData.toList()}');
  print('hashCode оригинала: ${originalData.hashCode}');
  print('Копия из воркера: ${copiedData.toList()}');
  print('hashCode копии: ${copiedData.hashCode}');
}

Future<void> worker(SendPort mainSendPort) async {
  ...
  await for (final message in port) {
    ...
    print('\nWorker Isolate:');
    print('Полученные данные: ${data.toList()}');
    print('hashCode полученной копии: ${data.hashCode}');

    // Возвращаем копию обратно
    replyPort.send(Uint8List.fromList(data));
  }
}

если передается originalData напрямую в sendPort.send(originalData), Dart под капотом сериализует весь объект и создаёт новый буфер в целевом изоляте.
Единственный способ «передать» обычный объект — скопировать его представление в сериализованном виде, а в целевом изоляте распаковать.
...
materialize() в TransferableTypedData — это метод, который «разворачивает» (материализует) переданные данные обратно в реальный доступный блок памяти, чтобы ты мог работать с ним как с обычным ByteBuffer/Uint8List.


Как это работает
Когда ты создаёшь TransferableTypedData и передаёшь его через изолят:
1) В исходном изоляте данные отцепляются от своего исходного буфера и становятся недоступны (владение памятью переходит).
2) В получающем изоляте ты получаешь «запакованный» объект TransferableTypedData, который ещё нельзя напрямую читать или писать.
...
- TransferableTypedData — это essentially «передача владения» буфером.
...
''',
  ...
  QA(
    ...
    a: 'Использование `await` внутри цикла последовательно ожидает завершения каждого Future, что может замедлить выполнение. Чтобы выполнять задачи параллельно, лучше собрать все Future в список и `await Future.wait()`.',
  ),
  QA(
    tags: [Tag.dart],
    q: 'Что такое рекурсия в Dart и есть ли ограничения по глубине?',
    a: '''
...
''',
  ...
  QA(
    tags: [Tag.dart],
    q: 'Что такое nullable и non-nullable типы в Dart?',
    a: 'Nullable типы могут содержать значение null (например, `String?`), non-nullable — не могут (например, `String`). Система null safety помогает предотвращать ошибки, связанные с null.',
  ),
  QA(
    tags: [Tag.dart],
...
''',
  ...
  QA(
    tags: [Tag.dart],
    q: 'В чем особенности синхронных и асинхронных генераторов в Dart?',
    a: 'Синхронный генератор (yield) возвращает последовательность элементов синхронно, асинхронный (async* и yield) — возвращает элементы по мере готовности, поддерживая await внутри.',
  ),
  QA(
    tags: [Tag.dart],
    q: 'Как работает final?',
    a: 'Final-переменная должна быть инициализирована ровно один раз. Это может происходить при объявлении, в конструкторе или через late инициализацию.',
  ),
  QA(
    tags: [Tag.dart],
...
''',
  ...
  QA(
    tags: [Tag.dart],
    q: 'Что такое `late` переменные и как они влияют на null safety?',
    a: '`late` позволяет отложить инициализацию переменной, при этом она не считается nullable, что помогает избежать проверок на null при гарантированной инициализации.',
  ),
  QA(
    tags: [Tag.dart],
...
В большинстве случаев this опционально и опускается для краткости
...
''',
  ...
  QA(
    tags: [Tag.dart],
    q: 'Какие существуют способы управления исключениями в Dart?',
    a: 'Использование блоков `try-catch-finally`, `on` для специфичных типов исключений, и `rethrow` для повторного проброса исключения с сохранением stack trace.',
  ),
  QA(
    tags: [Tag.dart],
    q: 'Что такое `deferred` загрузка в Dart?',
    a: 'Deferred загрузка позволяет загружать библиотеки по требованию, экономя ресурсы и уменьшая время запуска приложения. Используется через ключевое слово `deferred` при импорте.',
  ),
  QA(
    tags: [Tag.dart],
    q: 'Как в Dart реализовать ленивую инициализацию singleton?',
    a: 'С помощью `factory` конструктора и приватного статического поля, которое инициализируется при первом обращении (lazy initialization).',
  ),
  QA(
    tags: [Tag.dart],
    q: 'Что такое `call` метод в Dart и где он используется?',
    a: 'Метод `call` позволяет экземплярам классов вести себя как функции, позволяя вызывать объект как `instance()`.',
  ),
  QA(
    tags: [Tag.dart, Tag.stream],
    q: 'Что такое Stream в Dart?',
    a: 'Stream — это асинхронная последовательность событий (данных), которую можно слушать и обрабатывать по мере поступления.',
  ),
  QA(
    tags: [Tag.dart, Tag.stream],
    q: 'Чем Stream отличается от Future?',
    a: 'Future возвращает одно значение в будущем, а Stream — последовательность значений, поступающих асинхронно.',
  ),
  QA(
    q: 'Какие типы Stream бывают в Dart?',
    a: 'Single-subscription (одиночный подписчик) и broadcast (мультиподписочный) Stream.',
  ),
  QA(
    q: 'Когда использовать single-subscription Stream?',
    a: 'Когда поток должен иметь одного слушателя, например, для обработки результатов операций или передачи данных по одному потребителю.',
  ),
  QA(
    q: 'Когда использовать broadcast Stream?',
    a: 'Если нужно несколько слушателей для одного потока, например, для событий UI или оповещений.',
  ),
  QA(
  ...
  QA(
    q: 'Что такое StreamController и зачем он нужен?',
    a: '''
...
''',
  ...
  QA(
    q: 'Чем отличается синхронный и асинхронный StreamController?',
    a: 'Синхронный (`sync: true`) посылает события немедленно, асинхронный — через microtask, что влияет на порядок событий и производительность.',
  ),
  QA(
...
''',
  ...
  QA(
    ...
    a: 'Через параметр `onError` в `listen()`, или используя методы `handleError()`, `transform()` с кастомным StreamTransformer.',
  ),
  QA(
  ...
  QA(
    q: 'Какие есть способы создания Stream в Dart?',
    a: '''
// 1. Async generator
...
''',
  ...
  QA(
    q: 'Как реализовать трансформацию Stream с помощью StreamTransformer?',
    a: 'StreamTransformer позволяет определить логику преобразования, фильтрации или обработки событий и ошибок, создавая новый Stream из исходного.',
  ),
  QA(
    q: 'Какие типы Stream существуют?',
    a: '''
...
// Преобразование
Stream<int> stream = singleStream().asBroadcastStream();
''',
  ),
  QA(
    q: 'Что такое Future в Dart?',
  ...
  QA(
    q: 'Что такое async/await и как они связаны с Future?',
    a: '`async` помечает функцию как асинхронную, которая возвращает Future. `await` заставляет ждать завершения Future, упрощая синтаксис работы с асинхронным кодом.',
  ),
  QA(
  ...
  QA(
    q: 'Что такое цепочки Future и как они работают?',
  ...
  QA(
    ...
    a: 'Стандартный Future в Dart нельзя отменить напрямую. Для отмены используют дополнительные механизмы — например, Stream с поддержкой отмены или собственные контроллеры.',
  ),
  QA(
...
- Когда тебе нужно гарантированно вернуть Future, но функция может вернуть как T, так и Future<T>;
...
Необработанные ошибки Future становятся необработанными исключениями (unhandled exceptions), что может привести к завершению программы или предупреждениям во время выполнения.''',
  ...
  QA(
    q: 'Что такое event loop в Dart?',
    a: 'Event loop — это механизм, который управляет выполнением асинхронных задач, обрабатывая очередь событий и микротасков поочерёдно.',
  ),
  QA(
    q: 'Какие очереди существуют в Dart event loop?',
    a: 'Основные очереди: очередь микротасков (microtask queue) и очередь событий (event queue). Микротаски выполняются раньше, чем события.',
  ),
  QA(
    q: 'Что такое microtask в Dart?',
    a: 'Microtask — это небольшая асинхронная задача, которая выполняется до событийной очереди, используется для обработки Future и других async-операций.',
  ),
  QA(
  ...
  QA(
    q: 'Что такое Timer и в какую очередь он попадает?',
...
Пример с выводом порядка:

void main() {
  print('sync start');

  scheduleMicrotask(() => print('microtask via scheduleMicrotask'));
  Future.microtask(() => print('microtask via Future.microtask'));
  Future(() => print('event loop Future()'));
  Future.delayed(Duration.zero, () => print('event loop delayed'));
  Timer.run(() => print('event loop Timer.run'));
...
sync start
sync end
microtask via scheduleMicrotask
microtask via Future.microtask
event loop Future()
event loop delayed
event loop Timer.run

'''
        'С помощью `Future.delayed(Duration.zero)` или Timer с нулевой задержкой, которые ставят задачу в очередь событий.',
  ),
  QA(
...
''',
  ...
  QA(
    ...
    a: 'Когда микротаски постоянно добавляются и event loop не переходит к обработке событий, вызывая "голодание" событий.',
  ),
  QA(
  ...
  QA(
    ...
    a: 'В стандартной реализации отменить микротаск нельзя — нужно проектировать код с учётом возможности отмены через контролируемые флаги.',
  ),
  QA(
...
''',
  ...
  QA(
    q: 'Различия concurrency и параллелизм?',
    a: '''
...
''',
  ...
  QA(
    q: 'Как можно оптимизировать производительность при использовании Isolates?',
    a: 'Минимизировать объем передаваемых данных, переиспользовать Isolates, избегать частого создания и уничтожения.',
  ),
  QA(
    q: 'Что такое генераторы в Dart?',
    a: 'Генераторы — это функции, которые возвращают Iterable или Stream и используют ключевые слова `yield` или `yield*` для поэтапного создания последовательности данных.',
  ),
  QA(
  ...
  QA(
    q: 'Как использовать `yield` и `yield*` в генераторах?',
    a: '`yield` возвращает одно значение в последовательность, а `yield*` передает все значения из другой Iterable или Stream.',
  ),
  QA(
...
''',
  ...
  QA(
    q: 'Как настроить flavors для Android в Flutter проекте?',
    a: 'Через создание productFlavors в файле `android/app/build.gradle` с указанием разных applicationId, ресурсов и конфигураций.',
  ),
  QA(
    q: 'Как настроить flavors для iOS в Flutter проекте?',
    a: 'Через создание схем (schemes) и конфигураций (configurations) в Xcode с разными bundle identifiers и настройками.',
  ),
  QA(
    q: 'Как запускать приложение с определенным flavor через командную строку?',
    a: 'Для Android: `flutter run --flavor dev -t lib/main_dev.dart`. Для iOS: аналогично с указанием схемы и target файла.',
  ),
  QA(
...
''',
  ...
  QA(
    q: 'Можно ли использовать разные иконки и названия приложений для разных flavors?',
    a: 'Да, для Android и iOS можно указать разные иконки, имена приложений и даже launch screen через соответствующие ресурсы.',
  ),
  QA(
  ...
  QA(
    ...
    a: 'JIT — компиляция кода во время выполнения (runtime), что позволяет быстро запускать код и динамически изменять его.',
  ),
  QA(
    q: 'Что такое AOT-компиляция в Dart?',
    a: 'AOT — предварительная компиляция кода в машинный код до запуска, что улучшает производительность и уменьшает время старта приложения.',
  ),
  QA(
  ...
  QA(
    q: 'Что такое Isolate в Dart и чем он отличается от потока (thread) в других языках?',
    a: 'Isolate — это единица выполнения в Dart с собственной памятью и очередью событий. В отличие от потоков, Isolate не разделяют память: взаимодействие между ними возможно только через message passing, что исключает проблемы с race conditions.',
  ),
  QA(
    tags: [Tag.dart, Tag.isolate],
    q: 'Как создать новый Isolate и передать ему данные?',
    a: 'Используется функция Isolate.spawn(). Первый параметр — функция-обработчик, второй — данные. Данные должны быть сериализуемыми (например, JSON-объектами или примитивами).',
  ),
  QA(
    tags: [Tag.dart, Tag.isolate],
    q: 'Что произойдёт, если попытаться передать не сериализуемый объект в другой Isolate?',
    a: '''
При передаче не сериализуемого объекта в другой Isolate получишь ArgumentError с сообщением о том,
...
// Тяжелая функция - должна быть top-level
int heavyTask(int n) {
  int sum = 0;
  for (int i = 0; i < n; i++) {
    sum += i * i;
...
''',
  ...
  QA(
    tags: [Tag.dart],
    q: 'Что такое mixin в Dart и в каких случаях его стоит использовать?',
    a: r'''
...
mixin CacheableMixin<T> {
  ...
  Future<T> getOrCache(String key, Future<T> Function() fetcher) async {
...
class UserRepository with CacheableMixin<User> {
  Future<User> getUser(String id) {
    return getOrCache('user_$id', () async {
      // Fetch from API
...
Требуется горизонтальная композиция поведения
...
┌─────────────────────────────┬─────────────────────────────┬─────────────────────────────┐
│           Критерий          │            Mixin            │      Абстрактный класс      │
├─────────────────────────────┼─────────────────────────────┼─────────────────────────────┤
│ Наследование                │ Множественное (with)        │ Одиночное (extends)         │
├─────────────────────────────┼─────────────────────────────┼─────────────────────────────┤
...
''',
  ...
  QA(
    tags: [Tag.dart],
    q: 'Что делает ключевое слово late и зачем оно нужно?',
    a: '`late` говорит компилятору, что переменная будет инициализирована позже (не сразу), но точно один раз перед использованием.',
  ),
  QA(
    tags: [Tag.dart],
...
''',
  ...
  QA(
    tags: [Tag.dart],
    q: 'Что произойдёт, если указать required у позиционного параметра?',
    a: '`required` используется только с именованными параметрами. С позиционными это синтаксическая ошибка.',
  ),
  QA(
    tags: [Tag.dart],
    q: 'В чём разница между вызовами функции с именованными и позиционными параметрами?',
    a: 'Позиционные параметры передаются по порядку. Именованные — по имени, независимо от порядка (если они не позиционные). Именованные лучше читаются и масштабируются.',
  ),
  QA(
    tags: [Tag.dart],
    q: 'Как задать функцию с обязательными именованными параметрами и при этом указать default?',
    a: 'Так нельзя. Если параметр `required`, default значение не имеет смысла. Либо `required`, либо `=`.',
  ),
...
это класс, у которого определён метод call(...).
...
Dart позволяет передавать объект с методом `call` как функцию, если сигнатура совпадает. Например, `void doStuff(Function(String) action)` примет и `MyCallableClass().call`.

Если сигнатура `call()` совпадает с typedef, объект можно использовать там, где ожидается typedef-функция.

Недостатки:
- Снижение читаемости кода. Когда видишь instance(params), не всегда очевидно, что это не функция, а объект с методом call.
- Сложнее навигация по коду. В IDE "Go to definition" иногда ведёт в сам класс, но не сразу к call(), что сбивает с толку.
''',
  ),
  QA(
...
''',
  ...
  QA(
    ...
    a: 'Любые объекты, реализующие корректно `==` и `hashCode`. Обычно это `String`, `int`, `enum`, но можно и кастомные классы с переопределённым `==` и `hashCode`.',
  ),
  QA(
    tags: [Tag.dart],
...
Map.of(other)
То же, что Map.from, но проверяет совместимость типов ключей и значений с указанными generic-параметрами сразу при создании. Если хотя бы один элемент не подходит — выбросит CastError на месте.
...
''',
  ...
  QA(
    ...
    a: 'Через оператор `[]`, например: `final name = map["name"];`. Если ключ отсутствует — возвращается `null`. Можно задать значение по умолчанию: `map["key"] ?? "default"`.',
  ),
  QA(
    tags: [Tag.dart],
    q: 'Как удалить ключ из Map?',
    a: 'С помощью `map.remove("key")`. Также можно использовать `removeWhere((k, v) => condition)` для удаления по условию.',
  ),
  QA(
    tags: [Tag.dart],
...
MapEntry — это класс, представляющий пару ключ-значение в Map.
MapEntry — это просто контейнер для связки ключ-значение.
...
final set3 = Set.from(list); // {1, 2, 3}

// Из другого Set
final set4 = Set.of(set1);
...
final symmetric = setA.lookup(3) == null ? setA.union(setB) : setA.difference(setB);
''',
  ),
...
''',
  ...
  QA(
    ...
    a: 'abstract class может наследоваться откуда угодно, а sealed — только внутри той же библиотеки. Sealed также запрещает создание экземпляра и может быть использована для исчерпывающих проверок.',
  ),
  QA(
    tags: [Tag.dart],
  ...
  QA(
    ...
    a: 'Да. В Dart 3 sealed классы отлично сочетаются с pattern matching, так как позволяют компилятору проверять все возможные случаи.',
  ),
  QA(
    tags: [Tag.dart],
  ...
  QA(
    tags: [Tag.dart],
    q: 'Как sealed class используется при построении функциональных архитектур (например, bloc или pattern matching в UI)?',
    a: 'Sealed классы идеально подходят для описания состояний и событий: компилятор гарантирует, что все случаи рассмотрены. Это снижает риск пропущенных веток в обработке.',
  ),

  QA(
    tags: [Tag.dart],
    q: 'Что такое BehaviorSubject и чем он отличается от обычного StreamController?',
    a: '''
...
Обычный StreamController такого не делает.
''',
  ),
  QA(
    tags: [Tag.dart],
...
''',
  ...
  QA(
    tags: [Tag.dart],
    q: 'Можно ли итерировать по очереди Queue? Что произойдёт, если во время итерации изменять очередь?',
    a: '''
Да, по Queue можно итерировать с помощью for-in.
Однако изменение очереди (например, удаление/добавление элементов) во время итерации приведёт к ConcurrentModificationError.
''',
  ),
...
''',
  ...
  QA(
    tags: [Tag.dart],
    q: 'Чем отличается оператор >> от >>> в других языках, и что есть в Dart?',
    a: '''
>> — арифметический сдвиг вправо (с сохранением знака)
>>> — логический сдвиг вправо (заполняет нулями)
В Dart есть только >>, логический сдвиг >>> отсутствует.
''',
  ),
  QA(
...
2) Отмена подписок. Если вы подписываетесь на события, но не отписываетесь, ссылки на подписчиков остаются в памяти.
...
''',
  ...
  QA(
    tags: [Tag.dart, Tag.memory],
    q: 'Есть пустой массив. Длина массива известа. В этот массив добавляются циклом данные. Почему такой подход плохой и как лучше изменить?',
    a: '''
...
- Когда вы добавляете элементы в массив (например, через add), и он превышает текущую вместимость, создается новый массив большего размера, и все элементы копируются в него.
...
2) Ключ 'Apple' хешируется: Хеш-функция вычисляет число для ключа 'Apple'. Например, пусть хеш равен 7.
3) Находим индекс 7 в таблице.
4) Добавляем пару {'Apple': 4} в бакет с индексом 7.
...
Коллизия возникает, когда два разных ключа отображаются в один и тот же индекс в хеш-таблице.
В Dart коллизии разрешаются с помощью списков внутри бакетов.
Если коллизий слишком много, производительность падает.

Предположим, размер таблицы равен 5, и все ключи дают одинаковый индекс (0):
...
При поиске ключа 'D':
- Мы находим бакет с индексом 0.
- Затем проходим по всем элементам списка, чтобы найти нужный ключ.
...
✗ Не подходит для конкурентного доступа
...
Отсортированность гарантируется инвариантом BST,
а не зависимо от формы дерева!
...
''',
  ...
  QA(
    tags: [Tag.dart, Tag.isolate],
    q: 'Приведите простой пример создания изолята с ReceivePort / SendPort',
    a: r'''
...
void main() async {
  ...
  receivePort.listen((message) {
    if (message is int) {
      print('Результат вычисления: $message');
    } else if (message is String) {
      print('Сообщение от изолята: $message');
    }
  });
...
void _isolateEntryPoint(SendPort sendPort) {
  // Создаем порт для приема сообщений в изоляте
  final receivePort = ReceivePort();

  // Отправляем обратно порт для отправки сообщений в этот изолят
  sendPort.send('Изолят запущен');
  ...
  // Отправляем наш порт в основной изолят для двусторонней связи
...
''',
  ...
  QA(
    tags: [Tag.dart, Tag.isolate],
    q: 'Какие типы данных можно передавать в изоляты?',
    a: '''

// Простые типы
port.send(42);           // int
port.send(3.14);         // double
port.send(true);         // bool
port.send('строка');     // String

// Коллекции
port.send([1, 2, 3]);           // List
port.send({'key': 'value'});    // Map
port.send({1, 2, 3});           // Set

// Пользовательские объекты (должны быть сериализуемыми)
class Person {
  final String name;
  final int age;
  ...
  // Factory constructor для десериализации
...
// Передача пользовательского объекта
port.send(Person('Alice', 30).toJson());
...
В старых версиях Dart (до Dart 2.12, до введения null safety):

Null был объектом типа Null, который наследовался от Object:
...
''',
  ...
  QA(
    tags: [Tag.dart],
    q: '''
Что делает allMatches для String?
''',
    a: '''

String.allMatches ищет все непересекающиеся вхождения заданного шаблона (строки или RegExp) в другой строке и возвращает Iterable<Match>.
...
'x'.allMatches(a) — одно совпадение ("x" на позиции 0).
'o'.allMatches(a) — три совпадения ("o" на позициях 1, 2, 3).

Под капотом 'x'.allMatches(...) — это короткая форма для:
RegExp('x').allMatches(a)
...
- T - тип данных, который вернет Future


2. Получение Future
final Future<T> future = completer.future;
- Возвращает Future, связанный с Completer
- Этот Future можно передавать другим функциям
- Он будет ждать, пока не будет завершен через Completer
...
final result = names.join(', '); // "Alice, Bob, Charlie"


// Числа (автоматически преобразуются в строки)
final numbers = [1, 2, 3, 4];
final numbersString = numbers.join('-'); // "1-2-3-4"


// С пустым разделителем
final chars = ['H', 'e', 'l', 'l', 'o'];
final word = chars.join(''); // "Hello"
...
void main() {
  print(sumOf(3)); // Вывод: 1 + 2 + 3         = 6
  print(sumOf(5)); // Вывод: 1 + 2 + 3 + 4 + 5 = 15
}
...
O(1) Функция выполняет фиксированное количество операций, независимо от значения n:
...
Преимущества O(1)
Скорость: Не зависит от размера входных данных
Предсказуемость: Всегда выполняется за одно и то же время
...
- Все элементы меньше первого pivot
- Все элементы между pivot1 и pivot2
- Все элементы больше второго pivot
...
class User {
  ...
  // Именованные конструкторы - всегда создают новый объект
  ...
  User.child(String name) : this(name, 0); // Перенаправляющий
}
...
''',
  ...
  QA(
    tags: [Tag.dart, Tag.future],
    q: '''
В каком порядке выполнить этот код?
...
3. From constructor Microtask

// Затем EVENT QUEUE:
2. From constructor Future
7. Main Future
''',
  ),
  QA(
    tags: [Tag.dart, Tag.future],
    q: '''
В каком порядке выполнится этот код?
  import 'dart:async';
