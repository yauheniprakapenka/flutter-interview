...
const List<QA> dartInterviewQuestions = [
  QA(
    tags: [Tag.dart],
    q: 'Что такое аннотация @immutable?',
    a: '''
Используется из пакета meta. Она подсказывает анализатору кода, что все поля в классе должны быть final. Это стандарт для всех Widget во Flutter.
...
Поскольку мы не можем менять поля, мы используем паттерн copyWith. Это стандарт для State Management (Bloc, Riverpod).
''',
  ),
  QA(
    tags: [Tag.dart],
...
class Object {
	...
	external bool operator ==(Object other); // true если этот один и тот же объект
...
- Принадлежность: Принадлежат конкретному объекту. У каждого экземпляра класса своя копия полей.
- Жизненный цикл: Создаются при инициализации объекта и уничтожаются сборщиком мусора (GC), когда на объект больше нет ссылок.
- Доступ: Имеют доступ к ключевому слову this и другим instance-методам/полям.

Static (статические) члены:
- Принадлежность: Принадлежат самому классу как типу, а не его экземплярам.
- Ленивая инициализация (Важно для Dart): В Dart статические переменные инициализируются лениво — только в момент первого обращения к ним. Это экономит ресурсы при запуске приложения.
- Память: Хранятся в памяти на протяжении всего времени жизни приложения (точнее, до завершения работы изолята).
- Ограничения: Статические методы не имеют доступа к this и не могут напрямую обращаться к нестатическим полям класса, так как они существуют вне контекста конкретного объекта.
'''),
  QA(
    tags: [Tag.dart],
...
''',
  ...
  QA(
    ...
    a: '''
Создать immutable-объект можно с помощью `final` полей и `const` конструктора.

import 'package:meta/meta.dart';

@immutable // Подсказка анализатору: все поля ДОЛЖНЫ быть final
class User {
  final String id;
  final String name;
  ...
  // 2. Паттерн copyWith для "изменения" объекта через создание нового
...
''',
  ...
  QA(
    ...
    q: 'Что такое typedef?',
    a: '''
`typedef` используется для создания псевдонима типа.
...
- Добавлять поля, конструкторы, геттеры, сеттеры
- Определять методы
...
''',
  ...
  QA(
    tags: [Tag.dart],
    q: 'В чем разница между runZoned и runZonedGuarded?',
    a: r'''

Оба метода создают Zone — изолированный контекст выполнения в Dart, который позволяет:
- перехватывать ошибки,
- переопределять поведение (timers, microtasks, print и т.д.),
- передавать контекст вниз по async-цепочке.

Но между ними есть критические различия.
...
Характеристики:
- Перехватывает только синхронные ошибки
- Асинхронные ошибки (Future, async/await) НЕ гарантированно перехватываются
- Требует ручной и осторожной работы с onError

Используется в основном для кастомизации Zone, а не глобальной обработки ошибок

🔹 runZonedGuarded (рекомендуемый вариант)
...
- Перехватывает и синхронные, и асинхронные ошибки
- Полностью совместим с async/await
- Гарантирует, что любая необработанная ошибка внутри зоны попадёт в обработчик
- Стандарт для Flutter-приложений

🔹 Типичное использование во Flutter
...
🔹 Что runZonedGuarded НЕ перехватывает

❌ Ошибки в других изолятах

void main() {
  runZonedGuarded(() async {
    await Isolate.spawn(backgroundTask, 'data');
  }, (error, stack) {
    print('Ошибка в основном изоляте: $error');
  });
}

void backgroundTask(String data) {
  throw Exception('Ошибка в изоляте');
}

Почему:
- Isolate — это отдельный поток выполнения
- Zone работает только внутри одного изолята
- Каждый изолят имеет:
- собственный event loop
- собственные Zones
- собственную обработку ошибок

➡️ Ошибки изолятов нужно обрабатывать через:
- ReceivePort
- Isolate.addErrorListener
- или внутри самого изолята

🔹 Краткое сравнение

| Характеристика           | runZoned            | runZonedGuarded |
| ------------------------ | ------------------- | --------------- |
| Синхронные ошибки        | ✅                  | ✅              |
| Асинхронные ошибки       | ⚠️ не гарантировано | ✅              |
| async / await            | ❌                  | ✅              |
| Использование во Flutter | ❌ не рекомендуется | ✅ стандарт     |
...
''',
  ...
  QA(
    ...
    a: r'''
В Dart изолят — это полностью изолированный поток выполнения, со своей памятью, event loop и зонами.

Поэтому:

❗ Ошибки из другого изолята НЕ могут быть перехвачены try/catch, runZonedGuarded или FlutterError.onError из основного изолята.

Обработка ошибок в изолятах должна быть явной.

🔹 Базовый и самый распространённый способ — через SendPort / ReceivePort

Идея:
- ловим ошибку внутри изолята
- передаём её в основной изолят через порт сообщений
- обрабатываем централизованно
...
void main() async {
  ...
  receivePort.listen((message) {
    if (message is IsolateError) {
      ...
      // Логирование / Crashlytics / Sentry
    }
  });
}

📦 DTO для передачи ошибки
...
void backgroundTask(SendPort sendPort) {
  try {
    // Работа в изоляте
    throw Exception('Ошибка в фоновом изоляте');
  } catch (error, stackTrace) {
    sendPort.send(IsolateError(error, stackTrace));
  }
}
...
''',
  ...
  QA(
    ...
    a: r'''
FlutterError.onError — это глобальный обработчик ошибок Flutter framework, который ловит ошибки в build/layout/render фазах UI.
Он не перехватывает async или isolate ошибки и обычно используется вместе с runZonedGuarded для полного покрытия.

📌 Проще:
Это handler ошибок UI-фреймворка, а не всех ошибок приложения.

🔹 Что именно перехватывает FlutterError.onError

✅ Ошибки во Flutter framework:
- ошибки в build()
- layout / render ошибки
- ошибки в lifecycle виджетов
- assert-ошибки Flutter

❌ НЕ перехватывает:
- необработанные async ошибки (Future, async/await)
- ошибки в Dart Isolate
- ошибки до инициализации Flutter
- platform channel ошибки (напрямую)

🔹 Базовая настройка FlutterError.onError
...
void main() {
  FlutterError.onError = (FlutterErrorDetails details) {
    ...
    // Отправка в Crashlytics / Sentry
    FirebaseCrashlytics.instance.recordFlutterError(details);

    // В debug режиме — стандартный красный экран
    if (kDebugMode) {
...
📌 В production presentError обычно не вызывают, чтобы не показывать пользователю debug UI.

🔹 Связь с глобальной обработкой ошибок Dart
...
void main() {
  runZonedGuarded(() {
    ...
    // Асинхронные и не-FLutter ошибки
...
| Механизм             | Что ловит               |
| -------------------- | ----------------------- |
| FlutterError.onError | Ошибки Flutter UI       |
| runZonedGuarded      | Dart async ошибки       |

🔹 Кастомный экран ошибки (замена Red Screen of Death)

Во Flutter UI ошибки можно перехватить визуально через ErrorWidget.builder.
...
📌 Обычно:
- Debug — показываем детали
- Release — дружелюбный fallback UI
...
3) Вызов .materialize() отдаёт ByteBuffer, который и есть та же самая память, но теперь доступная в этом изоляте.
...
Изоляты принципиально не разделяют память, чтобы избежать race condition.
...
''',
  ...
  QA(
    ...
    a: 'Isolates — это отдельные потоки с собственной памятью. Они общаются друг с другом через обмен сообщениями (SendPort/ReceivePort), что исключает race condition.',
  ),
  QA(
    tags: [Tag.dart, Tag.future],
...
''',
  ...
  QA(
    ...
    a: '''
Рекурсия — вызов функции самой себя. Ограничения зависят от размера стека — слишком глубокая рекурсия приведёт к ошибке StackOverflowError.
...
''',
  ...
  QA(
      tags: [Tag.dart],
      q: 'Что такое shadowing (затенение) переменных в Dart?',
      a: '''
Shadowing (затенение) — это ситуация, когда переменная, объявленная в более узкой области видимости (например, внутри метода или блока),
имеет то же имя, что и переменная во внешней области видимости. В результате внутренняя переменная «затеняет» внешнюю, делая её недоступной по прямому имени.

void main() {
  String name = "Global"; // Внешняя переменная

  if (true) {
    String name = "Local"; // ЗАТЕНЕНИЕ: мы создали новую переменную с тем же именем
    print(name); // Выведет: "Local"
  }

  print(name); // Выведет: "Global" (внешняя переменная не изменилась!)
}
'''),
  QA(
    tags: [Tag.dart],
...
''',
  ...
  QA(
    tags: [Tag.dart],
    q: 'Что такое deferred (отложенная загрузка)?',
    a: '''
...
Позволяет уменьшить время загрузки. Полезно для A/B тестов, когда нужна только одна библиотека, а вторую зря не загружать.
''',
  ),
  QA(
    tags: [Tag.dart, Tag.stream],
...
''',
  ...
  QA(q: 'Как работает метод where() в Stream?', a: '''
Метод where() (также известный как фильтрация) создает новый поток на основе существующего, пропуская только те события, которые соответствуют заданному предикату.

// Поток нажатий на кнопку, где мы игнорируем пустые поисковые запросы
searchStream
  .map((event) => event.trim())
  .where((text) => text.length > 2) // Пропускаем только запросы длиннее 2 символов
  .listen((query) => performSearch(query));

'''),
  QA(q: 'Как работает метод take() в Stream?', a: '''
Метод take(n) ограничивает количество событий в потоке. После того как поток выдаст n элементов, подписка автоматически закрывается,
и поток (в контексте данного слушателя) прекращает выполнение.

// Нам нужно только первое валидное состояние пользователя для навигации
userStream
  .where((user) => user != null)
  .take(1) // Как только пользователь пришел, стрим для этого слушателя закроется
  .listen((user) => navigateToHome(user));
...
''',
  ...
  QA(q: 'Что такое cancelOnError в Stream?', a: r'''

cancelOnError — это механизм контроля жизненного цикла подписки. Eсли логика потока подразумевает,
что после ошибки состояние становится невалидным, необходимо указать true.

Если же это поток независимых событий, оставляем false и обрабатывываем ошибки через onError, чтобы сохранить работоспособност.

stream.listen(
  (data) => print('Step: $data'),
  onError: (err) {
    print('Critical failure: $err');
    // Стрим закроется сам сразу после этого
  },
  cancelOnError: true,
);
// Если придет: Шаг 1 -> Ошибка -> Шаг 2
// Результат: "Шаг 2" никогда не будет напечатан.
...
''',
  ...
  QA(
    ...
    a: '''
Методы first, last, single возвращают Future.
Как только условие выполняется, подписка обычно закрывается.

🎯 1. first и firstWhere (Дай мне самое первое и забудь)
Используются, когда нам нужно получить самое первое значение и сразу прекратить прослушивание.

// Пример: Получение текущих координат один раз
Future<Location> getCurrentLocation(Stream<Location> locationStream) async {
  // Мы берем первое же событие и отписываемся от обновлений GPS
  return await locationStream.first;
}

// Пример с условием:
final firstPositive = await stream.firstWhere((n) => n > 0);

🎯 2. last и lastWhere

Дожидаются закрытия стрима и возвращают последнее пришедшее значение.
code

// Пример: Получение финального состояния процесса
Future<Status> getFinalStatus(Stream<Status> statusStream) async {
  // Ждет, пока контроллер стрима вызовет .close()
  return await statusStream.last;
}

🎯 3. single (Убедись, что это единственный элемент)

Возвращает единственный элемент стрима.

Он берет первый элемент, но не завершается.[2] Он продолжает слушать стрим и ждет события done (закрытия стрима).
Он должен быть уверен, что после первого элемента больше ничего не придет.

Когда выдаст ошибку:
- Если стрим пустой.
- Если в стриме появился второй элемент. Как только приходит второй элемент, single мгновенно выбрасывает ошибку, так как условие «единственности» нарушено.

// Пример: Когда мы ожидаем ровно один результат (например, запрос к БД по ID)
Future<User> getSingleUser(Stream<User> userStream) async {
  return await userStream.single;
}
''',
  ),
  QA(
...
''',
  ...
  QA(
    tags: [Tag.dart, Tag.future, Tag.stream],
...
''',
  ...
  QA(
    tags: [Tag.dart, Tag.future, Tag.flutter],
...
''',
  ...
  QA(
    tags: [Tag.dart, Tag.isolate, Tag.future],
...
''',
  ...
  QA(
    ...
    a: '''
В Dart аналогом flatMap является комбинация методов expand.
...
await compute(fun, message) эквивалентно await Isolate.run(() => fun(message)).
...
''',
  ...
  QA(
    tags: [Tag.dart],
    q: 'Что делает Оператор Bitwise XOR (^) — "исключающее ИЛИ"',
    a: '''
...
''',
  ...
  QA(
    q: 'Concurrency и Parallelism',
    a: '''
🎯 Concurrency (Конкурентность):
- Это когда несколько задач выполняются вперемешку, поочередно, на одном ядре процессора.
- Задачи не работают одновременно, а быстро переключаются друг с другом (context switch).
- Цель — эффективное использование одного ресурса (например, одного ядра CPU), чтобы система могла отвечать на многое сразу.
Пример: один человек готовит обед, но то моет овощи, то ставит кастрюлю на плиту, то режет мясо — он делает всё по очереди, но создаёт впечатление, что делает всё сразу.
...
- Это когда несколько задач действительно выполняются одновременно, на разных физических ядрах или процессорах.
- Требует нескольких ресурсов (нескольких ядер CPU).
- Цель — ускорить выполнение задач за счёт параллельной работы.
- Пример: двое людей готовят обед вместе — один режет овощи, другой варит суп. Они работают одновременно и независимо.
...
Все три задачи выполняются ОДНОВРЕМЕННО — каждая на своём ядре.
''',
  ),
  QA(
    q: 'Сетевая модель OSI',
    a: '''
Сетевая модель OSI (Open Systems Interconnection) — это концептуальная модель, которая описывает,
как данные передаются по сети. Она состоит из 7 уровней, каждый из которых отвечает за определённую функцию.

Уровни идут от нижнего (физического) к верхнему (прикладному). Данные проходят сверху вниз при отправке и снизу вверх при получении.

🎯 1. Физический уровень (Physical Layer)
- Передаёт сырые биты (0 и 1) по физической среде: кабель, Wi-Fi, оптоволокно.
- Отвечает за напряжение, частоту, тип кабеля, разъёмы.
- Пример: Ethernet-кабель, радиоволны Wi-Fi.

🎯 2. Канальный уровень (Data Link Layer)
- Обеспечивает надёжную передачу данных между двумя устройствами в одной сети (например, между вашим ноутбуком и роутером).
- Использует MAC-адреса для идентификации устройств.
- Делит данные на кадры (frames), проверяет ошибки.
- Пример: Wi-Fi (802.11), Ethernet.

🎯 3. Сетевой уровень (Network Layer)
- Отвечает за маршрутизацию данных между разными сетями.
- Использует IP-адреса для определения, откуда и куда идут данные.
- Пример: протокол IP (IPv4, IPv6), маршрутизаторы.

🎯 4. Транспортный уровень (Transport Layer)
- Обеспечивает надёжную передачу данных между двумя устройствами (конечными точками).
- Управляет потоком, контролем ошибок, повторной передачей.
- Использует порты для адресации приложений.
- Пример: TCP (надёжный, с подтверждением), UDP (быстрый, без подтверждения).

🎯 5. Сеансовый уровень (Session Layer)
- Управляет сеансами связи: открывает, поддерживает и закрывает соединения между приложениями.
- Следит за тем, чтобы сеанс не прервался внезапно.
- Пример: сессии в веб-приложениях, RPC.

🎯 6. Уровень представления (Presentation Layer)
- Отвечает за формат данных: шифрование, сжатие, преобразование (например, из JPEG в PNG, или из UTF-8 в ASCII).
- Гарантирует, что данные понятны получателю.
- Пример: SSL/TLS (шифрование), кодеки.

🎯 7. Прикладной уровень (Application Layer)
- Интерфейс между пользователем/приложением и сетью.
- Здесь работают протоколы, с которыми взаимодействует пользователь.
- Пример: HTTP (веб), FTP (файлы), SMTP (почта), DNS.

Когда вы открываете сайт:

Ваш браузер (Application Layer) формирует HTTP-запрос.
- Presentation Layer может зашифровать его через TLS.
- Session Layer устанавливает сеанс с сервером.
- Transport Layer разбивает данные на сегменты и добавляет порты (например, 443 для HTTPS).
- Network Layer добавляет IP-адреса (ваш и сервера).
- Data Link Layer добавляет MAC-адреса (ваш и роутера).
- Physical Layer отправляет биты по кабелю или Wi-Fi.
''',
  ),
  QA(
    q: 'Нужно загрузить 10,000 элементов. Что использовать: Array или Set?',
    a: '''
Что важнее — вставка, поиск или просто цикл по элементам?

🎯 Массивы

Ключевые моменты:
- Элементы сохраняются в том же порядке, в котором вы их добавляете
- Доступ по индексу осуществляется за время O(1)
- Могут быть дубликаты
- Но проверка наличия элемента (.contains) выполняется за O(n)
...
- Им не важен порядок
- Вставка и поиск в среднем выполняются за O(1) из-за хеширования
...
Примерно для 10,000 элементов оба варианта всё ещё быстры.
Но если увеличить до 1 миллиона, множества полностью разгромят массивы.
...
''',
  ...
  QA(
    q: 'Что такое radix?',
    a: '''
radix (основание системы счисления) — это параметр, который используется при преобразовании строк в числа и чисел в строки.

🎯 1. Преобразование строки в число

// Двоичная система (radix = 2)
int binary = int.parse('1010', radix: 2); // 10

// Восьмеричная система (radix = 8)
int octal = int.parse('17', radix: 8); // 15

// Десятичная система (radix = 10, по умолчанию)
int decimal = int.parse('42'); // 42

// Шестнадцатеричная система (radix = 16)
...
String binary = number.toRadixString(2);   // "11111111"
String octal = number.toRadixString(8);    // "377"
String decimal = number.toRadixString(10); // "255"
String hex = number.toRadixString(16);     // "ff"
''',
  ),
  QA(q: 'Минимальный уровень библиотеки в Dart?', a: 'Файл'),
  QA(
    q: 'Как работает null при валидации в TextFormField?',
    a: '''
При возврате строки происходит ошибка валидации, а если вернулся null, то ошибки нет.
...
''',
  ...
  QA(
    q: 'Объясните систему модификаторов классов в Dart (class, base, interface, final, sealed, mixin, abstract)',
    a: '''
+---------------------------+---------------+---------+------------+-------+
| Запись                    | Конструктор   | Extends | Implements | Mixin |
+---------------------------+---------------+---------+------------+-------+
...
| abstract base class       | No            | Yes     | No         | No    |
| abstract interface class  | No            | No      | Yes        | No    |
| abstract final class      | No            | No      | No         | No    |
| mixin class               | Yes           | Yes     | Yes        | Yes   |
| base mixin class          | Yes           | Yes     | No         | Yes   |
| abstract mixin class      | No            | Yes     | Yes        | Yes   |
| abstract base mixin class | No            | Yes     | No         | Yes   |
| mixin                     | No            | No      | Yes        | Yes   |
| base mixin                | No            | No      | No         | Yes   |
+---------------------------+---------------+---------+------------+-------+
''',
  ),
  QA(
    q: 'чем отличаются от Uri.encodeComponent() / Uri.decodeComponent()',
    a: '''
Uri.encodeFull() / Uri.decodeFull() — кодируют/декодируют полный URI, сохраняя специальные символы URI (такие как /, :, ?, #, &).

Uri.encodeComponent() / Uri.decodeComponent() — кодируют/декодируют отдельные компоненты URI, экранируя ВСЕ специальные символы.

// encodeFull / decodeFull - для полных URI
const text1 = 'hello world';
final encoded1 = Uri.encodeFull(text1);
// Результат: 'hello%20world' (пробел закодирован)

const text2 = 'https://example.com/path?name=John Doe';
final encoded2 = Uri.encodeFull(text2);
// Результат: 'https://example.com/path?name=John%20Doe'
// Заметьте: :, /, ? остались без изменений

// encodeComponent / decodeComponent - для частей URI
const param = 'John Doe & Jane';
final encoded3 = Uri.encodeComponent(param);
// Результат: 'John%20Doe%20%26%20Jane' (& тоже закодирован)

const url = 'https://example.com/search?q=';
final query = 'cats & dogs';
final fullUrl = url + Uri.encodeComponent(query);
// Результат: 'https://example.com/search?q=cats%20%26%20dogs'

Когда использовать:
- encodeFull — когда у вас уже есть полный URL и нужно закодировать только "небезопасные" символы (пробелы, кириллицу и т.д.)
- encodeComponent — когда кодируете параметры запроса или части пути, где символы типа &, =, / должны быть экранированы

Распространенная ошибка:

// НЕПРАВИЛЬНО:
final url = Uri.encodeFull('https://example.com?name=A&B');
// Результат: 'https://example.com?name=A&B' (& не закодирован!)

// ПРАВИЛЬНО:
final url = 'https://example.com?name=' + Uri.encodeComponent('A&B');
// Результат: 'https://example.com?name=A%26B'
''',
  ),
  QA(
    q: 'Что такое функции высшего порядка в Dart?',
    a: r'''
Функция высшего порядка (Higher Order Function) – это функция, которая принимает другую функцию
в качестве аргумента или возвращает функцию в качестве результата.
...
class MyClass {
  void greetAgain({required Function f, required String name}) {
    f(name);
  }
}

main() {
  MyClass().greetAgain(f: greet, name: 'Tom'); // Hello, Tom
}
''',
  ),
  QA(
    q: 'Что такое оператор => (стрелочная функция) ?',
    a: r'''
=> — это синтаксический сахар для создания коротких функций, которые содержат только одно выражение.
Это сокращённая запись для функций, которые сразу возвращают результат.
...
📌 1. Неявный return
- После => автоматически возвращается результат выражения
- Нельзя использовать фигурные скобки {}
- Нельзя писать return явно

📌 2. Только одно выражение

// ✅ Правильно
String greet(String name) => 'Hello, $name!';

// ❌ Неправильно - несколько выражений
...
Короткие методы с одним выражением
Callback-функции
Лямбды для методов коллекций

❌ Не используй:

Когда нужно несколько строк кода
Когда логика сложная и нужны промежуточные переменные
Когда это ухудшает читаемость

Важно помнить

=> можно использовать только с выражениями (expressions), не с утверждениями (statements)
Нельзя использовать await, циклы, if-else блоки напрямую (но можно через тернарный оператор)
Код должен оставаться читаемым!
''',
  ),
  QA(
    q: 'В чём разница между циклами while и do-while ',
    a: '''
📌 цикл while проверяет условие до выполнения тела цикла, поэтому его тело может не выполниться ни разу,
если условие изначально ложно:
while (условие) {
  // тело цикла
}

📌 цикл do-while, напротив, сначала выполняет тело, а затем проверяет условие. Это гарантирует,
что тело цикла выполнится хотя бы один раз:

String? input;
do {
  stdout.write('Введите положительное число: ');
  input = stdin.readLineSync();
} while (input == null || int.tryParse(input) == null || int.parse(input) <= 0);

⚠️ Обратите внимание: в Dart нет цикла while do (как в Pascal или некоторых других языках)

📌 Оператор break прерывает выполнение цикла и передаёт управление первой инструкции после цикла.
...
(проверке условия в while или do-while, либо к следующему элементу в for).
...
''',
  ...
  QA(
    q: 'Как работает оператор break с метками (labels)?',
    a: r'''
📌 Оператор break может использоваться с метками (labels), чтобы выйти не только из ближайшего цикла,
но и из любого внешнего цикла, помеченного соответствующей меткой. Это особенно полезно при работе
с вложенными циклами, когда нужно прервать выполнение сразу нескольких уровней.
...
void main() {
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      if (i == 1 && j == 1) {
        break outerLoop; // выйти из внешнего цикла
      }
      print('i: $i, j: $j');
    }
  }
...
С меткой — управление передаётся сразу за пределы помеченного цикла.

Хотя использование меток возможно, в Dart (как и в большинстве современных языков) их рекомендуется применять редко,
так как это может ухудшить читаемость кода. Часто задачу можно решить через вынос логики в отдельную функцию с ранним return.
...
''',
  ...
  QA(
    q: 'Как работает оператор switch',
    a: r'''
Оператор switch поддерживает только точное сравнение на равенство (==). Он не может работать с диапазонами,
логическими выражениями вроде x > 5 или сложными условиями — для этого следует использовать if/else.

Каждый case в switch должен завершаться одним из управляющих операторов:

break — завершает выполнение switch,
return — выходит из функции,
throw — генерирует исключение,
continue — допускается только при использовании с метками (labeled switch), но редко применяется.
...
switch (weather) {
  case 'sunny':
    ...
    break;
  case 'cloudy':  // пустой case — разрешён
  case 'rainy':
...
если weather — 'cloudy' или 'rainy', выполнится одна и та же ветка. Такой подход часто используется
для группировки нескольких значений с одинаковой логикой.
''',
  ),
  QA(
    q: 'Для чего нужна структура папок lib/src/ ?',
    a: '''
Структура папок lib/src/ используется для внутренней реализации пакета — то есть для кода, который
не предназначен для прямого импорта внешними пользователями.

my_package/
├── lib/
│   ├── my_package.dart        ← публичный API (точка входа)
│   └── src/
│       ├── utils.dart
│       ├── core.dart
│       └── internal_stuff.dart
...
🔒 Зачем нужна папка src?

1. Сокрытие внутренней реализации
Всё, что находится в lib/src/, считается приватным для пакета. Пользователи пакета не должны импортировать файлы из src напрямую:

// ❌ ПЛОХО — нарушает инкапсуляцию
import 'package:my_package/src/utils.dart';

2. Контроль публичного API
Публичный интерфейс пакета экспортится только через главный файл (например, lib/my_package.dart):

// ✅ ХОРОШО — публичный API
import 'package:my_package/my_package.dart';

А внутри my_package.dart:

export 'src/core.dart';
export 'src/some_public_class.dart';
// но не экспортируем 'src/internal_helpers.dart'

3. Свобода рефакторинга

Поскольку src/ — приватная зона, автор пакета может:
- переименовывать файлы,
- менять структуру,
- удалять или переносить классы без нарушения обратной совместимости, пока публичный API (экспортируемый через lib/) остаётся неизменным.
''',
  ),
  QA(
    q: 'Как возвести число в степень?',
    a: '''
В Dart это делается с помощью функции pow() из библиотеки dart:math.

import 'dart:math';

void main() {
  print(pow(2, 3));   // 8   → 2³
  print(pow(5, 0.5)); // 2.236... → √5
  print(pow(10, -1)); // 0.1 → 10⁻¹
}

🔍 Возвращаемый тип

pow() возвращает num, но на практике это может быть int или double в зависимости от результата:

print(pow(2, 3).runtimeType);   // int
print(pow(2, 0.5).runtimeType); // double
''',
  ),
  QA(
    q: 'Что такое Queue?',
    a: '''
это очередь (FIFO — First In, First Out), то есть структура данных, где
элементы добавляются в конец, а извлекаются с начала.
...
+----------------------+----------------------------------------+--------------------------+
| add(element)         | Добавить в конец                       | queue.add(5)             |
| addAll(iterable)     | Добавить сразу несколько               | queue.addAll([6, 7])     |
| removeFirst()        | Удалить и вернуть первый элемент       | queue.removeFirst()      |
| removeLast()         | Удалить и вернуть последний элемент    | queue.removeLast()       |
| clear()              | Очистить очередь                       | queue.clear()            |
| isEmpty / isNotEmpty | Проверка                               | queue.isEmpty            |
| length               | Количество элементов                   | queue.length             |
+----------------------+----------------------------------------+--------------------------+

🧱 В чём отличие от List

+--------------------------------------+-----------------------------+----------------------------+
| Особенность                          | List                        | Queue                      |
+--------------------------------------+-----------------------------+----------------------------+
| Добавление и удаление в начало       | ❌ Медленное (O(n))         | ✅ Быстрое (O(1))          |
| Доступ по индексу                    | ✅ Есть (list[0])           | ❌ Нет                     |
| Идеально для                         | случайного доступа          | структуры FIFO             |
+--------------------------------------+-----------------------------+----------------------------+

👉 Поэтому если нужно часто добавлять и убирать элементы с начала и конца,
используй Queue, а не List.
''',
  ),
];
