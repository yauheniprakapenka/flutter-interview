...
''',
  ...
  QA(
    q: 'Что такое паттерн «Мост» (Bridge)?',
    a: r'''

Это структурный паттерн, который разделяет одну большую концепцию на две независимые иерархии: Абстракцию (логику верхнего уровня)
и Реализацию (платформенный или технический код).

В отличие от простого интерфейса, где мы просто подменяем одну реализацию другой, Мост позволяет обеим частям системы
развиваться параллельно, не увеличивая количество классов по экспоненте.


Представьте систему Логирования.

Ось Абстракции (Логика): SimpleLogger (только текст) и DetailedLogger (текст + время + стек вызовов).
Ось Реализации (Куда писать): ConsolePrinter, FilePrinter и SentryPrinter.

Вместо создания 6 классов (2 типа логики × 3 типа принтера), мы создаем «Мост».


// 1. Реализация (то, КАК выводим)
...
class ConsolePrinter implements LogPrinter {
  @override
  void printData(String data) => print('Console: $data');
}

class SentryPrinter implements LogPrinter {
  @override
  void printData(String data) => print('Sent to Sentry: $data');
}

// 2. Абстракция (то, ЧТО логируем - это и есть МОСТ)
abstract class BaseLogger {
  final LogPrinter printer; // Это поле — наш "мост" к реализации
  BaseLogger(this.printer);

  void log(String message);
}

class SimpleLogger extends BaseLogger {
  SimpleLogger(LogPrinter printer) : super(printer);

  @override
  void log(String message) => printer.printData(message);
}

class DetailedLogger extends BaseLogger {
  DetailedLogger(LogPrinter printer) : super(printer);

  @override
  void log(String message) {
    final enhancedMessage = "${DateTime.now()}: $message";
    printer.printData(enhancedMessage);
  }
}

void main() {
  // Мы можем комбинировать их как угодно в рантайме!
  final logger = DetailedLogger(SentryPrinter());
...
''',
  ...
  QA(
    ...
    a: '''
Компоновщик (Composite) — это структурный паттерн, который позволяет сгруппировать объекты в древовидную структуру
и работать с этой структурой так, будто это одиночный объект.

Основные роли:
1. Component (Интерфейс): Определяет общие методы для всех элементов (например, Widget).
2. Leaf (Лист): Простой элемент, у которого нет детей (например, Text, Image).
3. Composite (Композит): Сложный элемент, который содержит массив детей типа Component (например, Column, Container).

Реализация во Flutter:
- Flutter полностью построен на этом паттерне. Каждый раз, когда мы создаем дерево виджетов, мы используем Composite.
- Widget — это абстрактный интерфейс.
- Leaf-виджеты (например, Text) просто отрисовывают себя.
- Composite-виджеты (например, Row, Column, SizedBox) принимают child или children и делегируют им выполнение операций (отрисовку, расчет размеров).

''',
  ),
  QA(
    q: 'Что такое паттерн «Декоратор» (Decorator)?',
    a: '''
...
''',
  ...
  QA(
    ...
    a: '''
Flyweight — это структурный паттерн проектирования, который позволяет вместить большее количество объектов
в отведенную оперативную память. Он достигается за счет разделения состояния объекта на две части:

1. Внутреннее состояние (Intrinsic): Данные, которые одинаковы для множества объектов. Они хранятся внутри объекта и неизменяемы (immutable).
2. Внешнее состояние (Extrinsic): Контекстные данные, которые уникальны для каждого экземпляра. Они передаются извне в моменты вызова методов.

Flyweight направлен на минимизацию использования памяти путем разделения общего и уникального состояния.
В Dart это эффективно реализовано через const-канонизацию. В ситуациях, где значения не известны заранее, стоит реализовать этот паттерн через фабрику со static Map, к
оторая выступает в роли пула объектов. Это критически важно в Flutter для предотвращения лишних аллокаций памяти при частых перерисовках (rebuilds).


======

1. Реализация через const в Dart (Уникальная фишка)

- В Dart паттерн Flyweight встроен в компилятор. Когда мы используем const конструкторы, Dart выполняет канонизацию объектов.

- Если в разных частях программы мы создаем const EdgeInsets.all(8.0), Dart не создает новый объект каждый раз.
Он создает ровно один экземпляр в памяти и переиспользует ссылку на него везде. Это и есть идеальный Flyweight.

======

2. Ручная реализация (Flyweight Factory)

- Если данные приходят из сети или БД (мы не знаем их на этапе компиляции), мы используем фабрику с кэшированием.
Пример: Система иконок в игре или приложении, где тысячи элементов используют одни и те же тяжелые данные.

// Тяжелый объект (Flyweight)
class IconData {
  final String assetName; // Внутреннее состояние (общее)
  const IconData(this.assetName);
}

// Фабрика (Flyweight Factory)
class IconFactory {
  // Кэш для хранения существующих объектов
  static final Map<String, IconData> _icons = {};

  static IconData getIcon(String assetName) {
    // Если такая иконка уже создана — возвращаем её, а не создаем новую
    return _icons.putIfAbsent(assetName, () => IconData(assetName));
  }
}

// Использование
void main() {
  // Внешнее состояние (координаты X, Y) хранится отдельно
  final icon1 = IconFactory.getIcon('tree.png'); // Создается
  final icon2 = IconFactory.getIcon('tree.png'); // Берется из кэша

  print(identical(icon1, icon2)); // true — это один и тот же объект в памяти
}

1. Экономия RAM: Мы избегаем дублирования тяжелых данных (строк, бинарных данных, сложных структур).
2. Скорость сравнения: Поскольку объекты канонизированы (как const), мы можем сравнивать их по ссылке (быстро), а не по значениям полей (медленно).
3. Связь с Flutter: Во Flutter этот паттерн повсеместно используется в IconData, TextStyle, Color. Именно поэтому Flutter рекомендует ставить const перед виджетами — это напрямую снижает нагрузку на Garbage Collector (сборщик мусора).
''',
  ),
  QA(
    q: 'Что такое паттерн «Прокси» (Proxy)?',
    a: '''
Proxy — это структурный паттерн, который предоставляет объект-заменитель вместо реального объекта.
Прокси перехватывает вызовы к оригинальному объекту, позволяя выполнить какую-то логику до или после того, как вызов дойдет до реальной цели.

Он используется для выполнения дополнительных действий при обращении к объекту.

Например:
1. Модификация данных: Добавление служебных заголовков или токенов (как это делают Interceptors в библиотеке Dio).
2. Валидация: Проверка данных на корректность перед тем, как передать их в тяжелый метод бизнес-логики.
3. Трансформация: Приведение данных к нужному формату, который ожидает реальный объект».

Библиотека Dio и её Interceptors.

При вызове onRequest описываем прокси-логику. Перехватываем объект запроса RequestOptions, меняем в нем данные (добавляем токены)
и только потом вызываем handler.next(options), что означает «передать управление дальше реальному объекту».
''',
  ),
...
''',
  ...
  QA(
    q: 'Что такое паттерн «Абстрактная фабрика» (Abstract Factory)?',
    a: '''
...
''',
  ...
  QA(
    q: 'Что такое паттерн «Фабричный метод» (Factory Method)?',
    a: '''
...
''',
  ...
  QA(
    q: 'Что такое паттерн «Медиатор» (Mediator)?',
    a: r'''
