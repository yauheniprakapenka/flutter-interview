...
const List<QA> flutterInterview = [
  ...
  QA(
    ...
    a: '''
✅ 1. ValueKey<T>
Что делает: Сравнивает значение (==) и тип объекта.

Когда использовать: Когда есть уникальное примитивное значение (или объект с корректным ==),
...
✅ 2. ObjectKey
Что делает: Сравнивает сам объект по ссылке (identical(a, b)), игнорируя ==.
Когда использовать: Когда есть объект, который не переопределяет ==, или ты хочешь привязаться именно к экземпляру, а не к значению.
...
✅ 3. UniqueKey
Что делает: Гарантированно уникален — создаётся новый ключ при каждом создании экземпляра.

final key1 = UniqueKey();
final key2 = UniqueKey();
print(key1 == key2); // false — всегда

⚠️ Опасность: Использование UniqueKey в build() → новый ключ при каждом ререндере → Flutter думает, что это новый виджет → State теряется → виджет пересоздаётся с нуля → плохо для производительности и состояния.
✅ Идеально для:
...
''',
  ...
  QA(
    tags: [Tag.flutter],
    q: 'В каких случаях можно и нельзя использовать context в dispose()?',
    a: '''
❌ НЕЛЬЗЯ использовать context в dispose
Основное правило: НЕ используйте context в dispose() для операций с UI

class BadExampleWidget extends StatefulWidget {
  @override
  _BadExampleWidgetState createState() => _BadExampleWidgetState();
}

class _BadExampleWidgetState extends State<BadExampleWidget> {
  @override
  void dispose() {
    // ❌ НЕПРАВИЛЬНО - context может быть недействительным
    Navigator.of(context).pop(); // Может вызвать ошибку

    // ❌ НЕПРАВИЛЬНО - обращение к inherited widgets
    Theme.of(context); // Небезопасно
    MediaQuery.of(context); // Небезопасно

    // ❌ НЕПРАВИЛЬНО - показ диалогов/снэкбаров
...
✅ МОЖНО использовать context в dispose
Для доступа к сервисам и провайдерам (с осторожностью)

class GoodExampleWidget extends StatefulWidget {
  @override
  _GoodExampleWidgetState createState() => _GoodExampleWidgetState();
}

class _GoodExampleWidgetState extends State<GoodExampleWidget> {
  ...
  @override
  void dispose() {
    // ✅ ПРАВИЛЬНО - отписка от сервисов
    if (mounted) { // Проверяем что виджет все еще смонтирован
      Provider.of<SomeService>(context, listen: false).cleanup();
    }

    // ✅ ПРАВИЛЬНО - доступ к dependency injection
    if (mounted) {
      GetIt.instance<SomeService>().dispose();
    }
...
''',
  ...
  QA(
    ...
    a: r'''
✅ Сценарий: Один класс состояния (например, UserState с полями)
...
class UserState {
  ...
  // Копирующий конструктор для удобного изменения состояния
  UserState copyWith({
    User? user,
    bool? isLoading,
    String? error,
